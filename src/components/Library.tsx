...
import { friendlyFactionLabel, moderatorTraits, roleTeamLabel } from '../ui/labels'
...
export default function Library({ artifacts, roles, packs, scenarios, traitCatalogue, onRefresh, onEdit }: Props) {
  ...
  return <div className="library-page page-width">
    ...
    {tab === 'roles' && <><div className="filters"><label className="search"><Search /><input placeholder="Search names, traits, mechanics…" value={query} onChange={(event) => setQuery(event.target.value)} /></label><label><Filter /><select value={faction} onChange={(event) => setFaction(event.target.value)}><option value="all">All factions</option>{factions.map((item) => <option key={item} value={item}>{friendlyFactionLabel(item)}</option>)}</select></label><label><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">All mechanics</option>{categories.map((item) => <option key={item}>{item}</option>)}</select></label></div><div className="role-library-grid">{filtered.map((role) => <RoleCard key={`${role.id}-${role.meta.checksum}`} role={role} traitCatalogue={traitCatalogue} onEdit={() => onEdit(role)} />)}</div></>}
...
function RoleCard({ role, traitCatalogue, onEdit }: { role: RoleDefinition; traitCatalogue: TraitDefinition[]; onEdit: () => void }) {
  const traits = moderatorTraits(role.traits, traitCatalogue)
  return <article className="role-card"><header><div className={`faction-mark ${role.faction.split('.').at(-1)}`}><Shield /></div><div><h3>{role.meta.name}</h3><span>{roleTeamLabel(role)} · {role.meta.builtIn ? 'Built in' : `v${role.meta.version}`}</span></div></header><p>{role.text.summary}</p><div className="tag-row">{traits.map((trait) => <span className="tag trait-tag" style={{ borderColor: trait.colour, backgroundColor: `${trait.colour}1f` }} key={trait.id}><i style={{ backgroundColor: trait.colour }} />{trait.label}</span>)}</div><footer><button onClick={onEdit}>{role.meta.builtIn ? <><Copy /> View & clone</> : <>Edit definition</>}</button><button title="Export" onClick={() => downloadArtifact(role)}><Download /></button></footer></article>
}
