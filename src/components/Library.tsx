...
import { friendlyFactionLabel } from '../ui/labels'
...
function RoleCard({ role, traitCatalogue, onEdit }: { role: RoleDefinition; traitCatalogue: TraitDefinition[]; onEdit: () => void }) { const traits = role.traits.slice(0, 3).map((id) => traitCatalogue.find((trait) => trait.id === id) ?? { id, label: id.split('.').at(-1)?.replace(/-/g, ' ') ?? id, colour: '#8c857b' }); const team = role.displayTeam ?? friendlyFactionLabel(role.faction); return <article className="role-card"><header><div className={`faction-mark ${role.faction.split('.').at(-1)}`}><Shield /></div><div><h3>{role.meta.name}</h3><span>{team} · {role.meta.builtIn ? 'Built in' : `v${role.meta.version}`}</span></div></header><p>{role.text.summary}</p><div className="tag-row">{role.categories.slice(0, 3).map((tag) => <span className="tag" key={tag}>{tag}</span>)}{traits.map((trait) => <span className="tag trait-tag" style={{ borderColor: trait.colour, backgroundColor: `${trait.colour}1f` }} key={trait.id}><i style={{ backgroundColor: trait.colour }} />{trait.label}</span>)}</div><footer><button onClick={onEdit}>{role.meta.builtIn ? <><Copy /> View & clone</> : <>Edit definition</>}</button><button title="Export" onClick={() => downloadArtifact(role)}><Download /></button></footer></article> }
