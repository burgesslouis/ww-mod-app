...
export default function SetupWizard({ roles, packs, scenarios, initialSetup, onCancel, onStart }: Props) {
  ...
  const reconciledInformation = reconcileAbsentRoleSelections(setup, selectedRoles, scenario)
  setup.absentRoleSelections = reconciledInformation
  const informationChanged = JSON.stringify(reconciledInformation) !== JSON.stringify(absentRoleSelections)
  const validation = validateSetup(setup)

  useEffect(() => { window.scrollTo({ top: 0 }) }, [step])
  useEffect(() => {
    if (informationChanged) {
      setAbsentRoleSelections(reconciledInformation)
      setInformationNotice('The roles changed. Choices that are no longer eligible have been removed. Review the information before starting.')
    }
  }, [informationChanged, reconciledInformation])
  useEffect(() => {
  ...
  return <div className="setup-page page-width">
    ...
    <section className="setup-card">
      ...
      {step === 2 && <>
        ...
        <details className="faction-policy-option">
          <summary><div><span className="section-kicker">OPTIONAL</span><strong>Alternate win conditions</strong><small>Change which side a selected role wins with for this game.</small></div><ChevronDown /></summary>
          <div className="faction-policy-list">{activeRoles.map((role) => { const currentFaction = resolvedFactionPolicies?.[role.id]?.faction ?? role.faction; const recommendedFactions = [...(role.factionRecommendations ?? [])].filter((entry) => (!entry.when || publicRequirementsMet(entry.when, publicSetupFacts, selectedRoles)) && possibleFactionSet.has(entry.faction)); const recommendedIds = new Set(recommendedFactions.map((entry) => entry.faction)); const otherFactions = possibleFactionIds.filter((faction) => faction !== role.faction && !recommendedIds.has(faction)); const reason = recommendedFactions.find((entry) => entry.faction === currentFaction)?.reason; return <article className="faction-policy-row" key={role.id}>
            <div><strong>{role.meta.name}</strong><small>Normally wins with {factionNames.get(role.faction) ?? role.faction.split('.').at(-1)}</small></div>
            <label className="faction-policy-control"><span>Wins as</span><select aria-label={`${role.meta.name} wins as`} value={currentFaction} onChange={(event) => updateFactionPolicy(role.id, event.target.value)}><option value={role.faction}>{factionNames.get(role.faction) ?? role.faction.split('.').at(-1)} · role default</option>{recommendedFactions.filter((entry) => entry.faction !== role.faction).length > 0 && <optgroup label="Recommended for this setup">{recommendedFactions.filter((entry) => entry.faction !== role.faction).map((entry) => <option key={entry.faction} value={entry.faction}>{factionNames.get(entry.faction) ?? entry.faction.split('.').at(-1)}</option>)}</optgroup>}{otherFactions.length > 0 && <optgroup label="Other publicly possible factions">{otherFactions.map((faction) => <option key={faction} value={faction}>{factionNames.get(faction) ?? faction.split('.').at(-1)}</option>)}</optgroup>}</select><em>{resolvedFactionPolicies?.[role.id]?.winScope === 'alignment' ? 'Whole alignment' : 'Exact faction'}{reason ? ` · ${reason}` : ''}</em></label>
          </article> })}</div>
        </details>
      </>}
      ...
      {step === 3 && <>
        ...
        {informationRequirements.length > 0 && <section className="setup-information" aria-label="Role information">
          <h3>Role information</h3>
          {informationNotice && <p role="status" className="allocation-notice">{informationNotice}</p>}
          {informationRequirements.map(requirement => {
            const selected = reconciledInformation[requirement.roleId]?.[requirement.abilityId] ?? []
            return <fieldset key={`${requirement.roleId}:${requirement.abilityId}`}>
              <legend>{requirement.roleName} · {requirement.abilityName}</legend>
              <p>Choose at least {requirement.minimum} possible roles that are not in the deck. Tell the player these roles on Night 0.</p>
              <div className="prepared-role-options">{informationCandidates.map(id => {
                const name = selectedRoles.find(role => role.id === id)!.meta.name
                return <label key={id} className={selected.includes(id) ? 'selected' : ''}>
                  <input type="checkbox" checked={selected.includes(id)} aria-label={`${requirement.roleName}: reveal ${name}`} onChange={() => setAbsentRoleSelections(current => ({ ...current, [requirement.roleId]: { ...current[requirement.roleId], [requirement.abilityId]: selected.includes(id) ? selected.filter(value => value !== id) : [...selected, id] } }))} />
                  <span>{name}</span>
                </label>
              })}</div>
              <p aria-live="polite">{selected.length} selected · minimum {requirement.minimum}</p>
              {informationCandidates.length < requirement.minimum && <p className="error-banner">Not enough absent roles. Go back and add more possible roles, or change the deck.</p>}
            </fieldset>
          })}
        </section>}
