...
import type { AbilityDefinition, AttackModifiers, Effect, PackDefinition, PublicRequirement, RoleDefinition, ScenarioDefinition, TraitDefinition, TriggerType } from '../domain/types'
...
const TRIGGERS: TriggerType[] = ['setup.action', 'night.action', 'vote.beforeTally', 'vote.afterTally', 'ballot.qualified', 'burn.resolving', 'burn.resolved', 'attack.attempted', 'attack.successful', 'attack.hit', 'attack.redirected', 'attack.resolving', 'attack.prevented', 'kill.attempted', 'kill.redirected', 'kill.prevented', 'death.resolved', 'morning.beforeVictory', 'morning.announcements', 'victory.check']
...
function RoleBuilder({ role, traitCatalogue, editable, onChange }: { role: RoleDefinition; traitCatalogue: TraitDefinition[]; editable: boolean; onChange: (role: RoleDefinition) => void }) {
  ...
  return <div className="builder-layout"><div className="builder-main">
    ...
    <section className="editor-section"><div className="section-title"><div><span className="section-kicker">ABILITIES</span><h2>Triggers, conditions & effects</h2><p>Abilities run in order when their trigger occurs.</p></div></div><div className="template-row">{TEMPLATES.map((template) => <button key={template.name} disabled={!editable} onClick={() => addTemplate(template)}><Plus /> {template.name}</button>)}</div><div className="ability-list">{role.abilities.map((ability, index) => <AbilityCard key={ability.id} ability={ability} traitCatalogue={traitCatalogue} editable={editable} onChange={(next) => patch({ abilities: role.abilities.map((item, itemIndex) => itemIndex === index ? next : item) })} onDelete={() => patch({ abilities: role.abilities.filter((_, itemIndex) => itemIndex !== index) })} />)}</div></section>
...
function AbilityCard({ ability, traitCatalogue, editable, onChange, onDelete }: { ability: AbilityDefinition; traitCatalogue: TraitDefinition[]; editable: boolean; onChange: (ability: AbilityDefinition) => void; onDelete: () => void }) {
  ...
  const isAction = ability.kind === 'active' || ability.kind === 'shared-faction'
  const requirements = ability.publicRequirements?.requirements ?? []
  ...
  function setRequirements(next: PublicRequirement[], match = ability.publicRequirements?.match ?? 'all') {
    const changed = { ...ability }
    if (next.length) changed.publicRequirements = { match, requirements: next }
    else delete changed.publicRequirements
    onChange(changed)
  }
  function newRequirement(kind: PublicRequirement['kind'] = 'rolePossible'): PublicRequirement {
    if (kind === 'traitPossible') return { kind, trait: traitCatalogue[0]?.id ?? 'my.pack.trait.required' }
    if (kind === 'factionPossible') return { kind, faction: 'wherewolf.base.faction.village' }
    if (kind === 'packSelected') return { kind, packId: 'my.pack' }
    return { kind, roleId: 'my.pack.role.required' }
  }
  function requirementValue(requirement: PublicRequirement): string {
    if (requirement.kind === 'traitPossible') return requirement.trait
    if (requirement.kind === 'factionPossible') return requirement.faction
    if (requirement.kind === 'packSelected') return requirement.packId
    return requirement.roleId
  }
  function updateRequirement(index: number, requirement: PublicRequirement) { setRequirements(requirements.map((item, itemIndex) => itemIndex === index ? requirement : item)) }
  function patchTurnDependency(change: Partial<NonNullable<AbilityDefinition['turnDependency']>>) {
    if (!editable) return
    const current = ability.turnDependency ?? { anchors: { kind: 'roleTrait', trait: traitCatalogue[0]?.id ?? 'my.pack.trait.anchor' }, placement: 'before' as const, requiresSpokenCall: true }
    onChange({ ...ability, turnDependency: { ...current, ...change } })
  }
  function patchAttackModifiers(change: Partial<AttackModifiers>) {
    const effects = ability.effects.map((effect) => {
      if (effect.type !== 'queueAttack' && effect.type !== 'kill') return effect
      const current = effect.modifiers ?? {}
      const modifiers = { ...current, ...change }
      delete modifiers.witchProtectable
      return { ...effect, modifiers }
    })
    setEffectsJson(JSON.stringify(effects, null, 2)); onChange({ ...ability, effects })
  }
  const attackEffect = ability.effects.find((effect): effect is Extract<Effect, { type: 'queueAttack' | 'kill' }> => effect.type === 'queueAttack' || effect.type === 'kill')
  const attackModifiers = attackEffect?.modifiers ?? {}
  const guardable = attackModifiers.guardable ?? attackEffect?.type === 'queueAttack'
  const protectable = attackModifiers.protectable ?? attackModifiers.witchProtectable ?? attackEffect?.type === 'queueAttack'
  const healable = attackModifiers.healable ?? (attackEffect?.type === 'queueAttack' ? attackEffect.healable : false)
  const notifyTargetOnHit = attackModifiers.notifyTargetOnHit ?? (attackEffect?.type === 'queueAttack' ? attackEffect.notifyTargetOnHit : false)
  return <article className="ability-card">
    <header onClick={() => setOpen((value) => !value)}><span className={`ability-kind ${ability.kind}`}>{technicalLabel(ability.kind)}</span><div><strong>{ability.name}</strong><small>{technicalLabel(ability.trigger)} · {ability.effects.length} effect{ability.effects.length === 1 ? '' : 's'}</small></div><ChevronDown className={open ? 'rotated' : ''} /></header>
    {open && <div className="ability-body">
      <div className="form-grid">
        ...
        <label className="field"><span>Trigger</span><select disabled={!editable} value={ability.trigger} onChange={(event) => { const trigger = event.target.value as TriggerType; const ordered = trigger === 'setup.action' || trigger === 'night.action'; onChange({ ...ability, trigger, dependencyBarrier: ordered ? ability.dependencyBarrier : undefined, simultaneous: ordered ? ability.simultaneous : undefined }) }}>{TRIGGERS.map((trigger) => <option key={trigger} value={trigger}>{technicalLabel(trigger)}</option>)}</select></label>
        <label className="field"><span>Kind</span><select disabled={!editable} value={ability.kind} onChange={(event) => onChange({ ...ability, kind: event.target.value as AbilityDefinition['kind'] })}><option value="active">Active</option><option value="passive">Passive</option><option value="shared-faction">Shared faction</option><option value="status">Status</option></select></label>
        ...
        {isOrderedAction && <label className="field wide"><span>Spoken action</span><input disabled={!editable} value={ability.callout ?? ''} placeholder="check a player for corruption" onChange={(event) => onChange({ ...ability, callout: event.target.value || undefined })} /><small>This follows “Role, wake up and…” during a read-aloud night.</small></label>}
        {ability.trigger === 'night.action' && <div className="field wide dependency-editor"><span>Spoken-turn dependency</span><label className="check-row"><input type="checkbox" disabled={!editable} checked={Boolean(ability.turnDependency?.requiresSpokenCall)} onChange={(event) => { if (event.target.checked) patchTurnDependency({ requiresSpokenCall: true }); else { const next = { ...ability }; delete next.turnDependency; onChange(next) } }} /><span>This action requires its anchor role’s spoken call</span></label>{ability.turnDependency && <div className="form-grid"><label className="field"><span>Placement</span><select disabled={!editable} value={ability.turnDependency.placement} onChange={(event) => patchTurnDependency({ placement: event.target.value as 'before' | 'after' })}><option value="before">Before the anchor</option><option value="after">After the anchor</option></select></label><label className="field"><span>Anchor type</span><select disabled={!editable} value={ability.turnDependency.anchors.kind} onChange={(event) => patchTurnDependency({ anchors: event.target.value === 'roleTrait' ? { kind: 'roleTrait', trait: traitCatalogue[0]?.id ?? 'my.pack.trait.anchor' } : { kind: 'abilityIds', abilityIds: [] } })}><option value="roleTrait">Role trait</option><option value="abilityIds">Ability IDs</option></select></label>{ability.turnDependency.anchors.kind === 'roleTrait' ? <label className="field wide"><span>Trait anchor</span><input disabled={!editable} value={ability.turnDependency.anchors.trait} onChange={(event) => patchTurnDependency({ anchors: { kind: 'roleTrait', trait: event.target.value } })} /></label> : <label className="field wide"><span>Ability IDs (comma separated)</span><input disabled={!editable} value={ability.turnDependency.anchors.abilityIds.join(', ')} onChange={(event) => patchTurnDependency({ anchors: { kind: 'abilityIds', abilityIds: event.target.value.split(',').map((value) => value.trim()).filter(Boolean) } })} /></label>}</div>}</div>}
        {isAction && <div className="field wide public-requirements"><span>Public setup requirements</span><small>The app only shows, calls, or runs this action when these requirements are possible from the announced role pool. It never checks the dealt roles.</small>
          {requirements.length > 0 && <label><span>Require</span><select aria-label="Requirement matching" disabled={!editable} value={ability.publicRequirements?.match ?? 'all'} onChange={(event) => setRequirements(requirements, event.target.value as 'all' | 'any')}><option value="all">Every requirement</option><option value="any">At least one requirement</option></select></label>}
          <div className="public-requirement-list">{requirements.map((requirement, index) => <div key={index}>
            <select aria-label={`Public requirement ${index + 1} type`} disabled={!editable} value={requirement.kind} onChange={(event) => updateRequirement(index, newRequirement(event.target.value as PublicRequirement['kind']))}><option value="rolePossible">Role is possible</option><option value="traitPossible">Role with trait is possible</option><option value="factionPossible">Faction is possible</option><option value="packSelected">Pack is attached</option></select>
            <input aria-label={`Public requirement ${index + 1} reference`} disabled={!editable} value={requirementValue(requirement)} onChange={(event) => updateRequirement(index, requirement.kind === 'traitPossible' ? { ...requirement, trait: event.target.value } : requirement.kind === 'factionPossible' ? { ...requirement, faction: event.target.value } : requirement.kind === 'packSelected' ? { ...requirement, packId: event.target.value } : { ...requirement, roleId: event.target.value })} list={requirement.kind === 'traitPossible' ? `traits-${ability.id}` : undefined} />
            {editable && <button aria-label={`Remove public requirement ${index + 1}`} onClick={() => setRequirements(requirements.filter((_, itemIndex) => itemIndex !== index))}><Trash2 /></button>}
          </div>)}</div>
          {traitCatalogue.length > 0 && <datalist id={`traits-${ability.id}`}>{traitCatalogue.map((trait) => <option key={trait.id} value={trait.id}>{trait.label}</option>)}</datalist>}
          {editable && <button className="secondary add-public-requirement" onClick={() => setRequirements([...requirements, newRequirement()])}><Plus /> Add requirement</button>}
        </div>}
      ...
      {attackEffect && <div className="attack-hit-editor"><strong>Attack modifiers</strong><small>These flags describe what this effect can interact with. They are data, not role-specific engine rules.</small><label className="check-row"><input type="checkbox" disabled={!editable} checked={guardable} onChange={(event) => patchAttackModifiers({ guardable: event.target.checked })} /><span>Guardable (Guardian may redirect it)</span></label><label className="check-row"><input type="checkbox" disabled={!editable} checked={protectable} onChange={(event) => patchAttackModifiers({ protectable: event.target.checked })} /><span>Protectable by attack-protection effects</span></label><label className="check-row"><input type="checkbox" disabled={!editable} checked={healable} onChange={(event) => patchAttackModifiers({ healable: event.target.checked })} /><span>Healable hit</span></label><label className="check-row"><input type="checkbox" disabled={!editable} checked={notifyTargetOnHit} onChange={(event) => patchAttackModifiers({ notifyTargetOnHit: event.target.checked })} /><span>Tap the target when a healable hit gets through</span></label></div>}
...
function ContainerBuilder({ artifact, editable, onChange }: { artifact: PackDefinition | ScenarioDefinition; editable: boolean; onChange: (artifact: Artifact) => void }) {
  ...
  return <section className="editor-section container-builder"><span className="section-kicker">{artifact.meta.kind.toUpperCase()} IDENTITY</span><div className="form-grid"><label className="field wide"><span>Name</span><input disabled={!editable} value={artifact.meta.name} onChange={(event) => onChange({ ...artifact, meta: { ...artifact.meta, name: event.target.value } })} /></label><label className="field wide"><span>Description</span><textarea disabled={!editable} value={artifact.description} onChange={(event) => onChange({ ...artifact, description: event.target.value })} /></label></div>{isPack ? <><h2>Embedded roles</h2><p>Pack exports embed every role below, so the file is self-contained.</p><div className="embedded-list">{artifact.roles.map((role) => <div key={role.id}><strong>{role.meta.name}</strong><span>{role.displayTeam ?? friendlyFactionLabel(role.faction)}</span></div>)}</div></> : <><h2>Phase pipelines</h2><p>Use Advanced JSON for typed phase, capability, victory and dependency editing.</p><div className="pipeline-preview"><div><strong>One-time setup</strong>{artifact.setupPipeline.map((phase) => <span key={phase.id}>{phase.label} <small>{technicalLabel(phase.type)}</small></span>)}</div><div><strong>Repeating cycle</strong>{artifact.cyclePipeline.map((phase) => <span key={phase.id}>{phase.label} <small>{technicalLabel(phase.type)}</small></span>)}</div></div></>}</section>
}
