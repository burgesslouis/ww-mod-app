...
function role(id: string, name: string, faction: string, categories: string[], traits: string[], summary: string, description: string, abilities: AbilityDefinition[] = [], state: StateVariable[] = [], requirements: string[] = [], maximumCopies = 1): RoleDefinition {
  return withChecksum({
    ...
    faction, factionWinScope: faction === FACTION.anyShadow || faction === FACTION.anyHuman || faction === FACTION.village ? 'alignment' : 'exact', categories, traits, traitDefinitions: traits.map((traitId) => BASE_TRAITS.find((trait) => trait.id === traitId) ?? { id: traitId, label: traitId.split('.').at(-1) ?? traitId, colour: '#8c857b' }), multiplicity: { min: 0, max: maximumCopies },
...
const wolfBite: AbilityDefinition = {
  ...
  effects: [{ type: 'queueAttack', targets: { kind: 'chosen' }, attackType: 'shadow', modifiers: { guardable: true, healable: true, protectable: true, notifyTargetOnHit: true } }],
  callout: 'choose a player to bite', instructions: 'Choose any living player to bite, including a member of the Wolf Pack, or choose nobody.', requires: ['shadow-attacks'], dependencyBarrier: 'after-protection',
}
...
const roles: RoleDefinition[] = [
  role(ROLE.alphaWolf, 'Alpha Wolf', FACTION.wolves, ['Wolf Pack', 'Attack'], [TRAIT.werewolf, TRAIT.wolfAttacker, TRAIT.corrupt, TRAIT.shadow],
    'Leads the Wolf Pack and orders the bite.', 'On N0, learn the Pack and Defector. From N1, choose the Pack’s bite target.', [wolfIntro, { ...wolfBite, order: 50 }]),
  ...
  role(ROLE.clairvoyant, 'Clairvoyant', FACTION.village, ['Information', 'Night'], [TRAIT.mystic],
    'Checks whether one player is Corrupt.', 'Each night, select one living player and privately learn whether they have the Corrupt trait.', [
      { id: `${ROLE.clairvoyant}.check`, name: 'Read corruption', kind: 'active', trigger: 'night.action', order: 10, target: { label: 'Player to check', min: 1, max: 1, selector: { kind: 'allPlayers', life: 'alive' } }, effects: [{ type: 'inspectTrait', targets: { kind: 'chosen' }, trait: TRAIT.corrupt, positive: 'CORRUPT', negative: 'NOT CORRUPT', rememberAs: 'last-clairvoyant-corrupt' }], callout: 'check a player for corruption', instructions: 'Choose one living player to test for corruption.', requires: ['private-information'], dependencyBarrier: 'information' },
      { id: `${ROLE.clairvoyant}.setup-check`, name: 'N0 corruption read', kind: 'active', trigger: 'setup.action', order: 10, target: { label: 'Player to check', min: 1, max: 1, selector: { kind: 'allPlayers', life: 'alive' } }, effects: [{ type: 'inspectTrait', targets: { kind: 'chosen' }, trait: TRAIT.corrupt, positive: 'CORRUPT', negative: 'NOT CORRUPT', rememberAs: 'last-clairvoyant-corrupt' }], callout: 'check a player for corruption', instructions: 'Choose one living player to test for corruption on N0.', requires: ['private-information'], dependencyBarrier: 'information' },
    ]),
  role(ROLE.wizard, 'Wizard', FACTION.village, ['Information', 'Night'], [TRAIT.mystic],
    'Checks whether one player is Mystic.', 'Each night, select one living player and privately learn whether they have the Mystic trait.', [
      { id: `${ROLE.wizard}.check`, name: 'Read mysticism', kind: 'active', trigger: 'night.action', order: 20, target: { label: 'Player to test', min: 1, max: 1, selector: { kind: 'allPlayers', life: 'alive' } }, effects: [{ type: 'inspectTrait', targets: { kind: 'chosen' }, trait: TRAIT.mystic, positive: 'MYSTIC', negative: 'NOT MYSTIC' }], callout: 'check a player for mysticism', instructions: 'Choose one living player to test for mysticism.', requires: ['private-information'], dependencyBarrier: 'information' },
      { id: `${ROLE.wizard}.setup-check`, name: 'N0 mysticism read', kind: 'active', trigger: 'setup.action', order: 20, target: { label: 'Player to test', min: 1, max: 1, selector: { kind: 'allPlayers', life: 'alive' } }, effects: [{ type: 'inspectTrait', targets: { kind: 'chosen' }, trait: TRAIT.mystic, positive: 'MYSTIC', negative: 'NOT MYSTIC' }, { type: 'conditional', condition: { op: 'packSelected', packId: HIDDEN_PACK_ID }, effects: [{ type: 'learnPresence', targets: { kind: 'faction', faction: FACTION.inquisition, life: 'any' }, label: 'Inquisition' }] }], callout: 'check a player for mysticism', instructions: 'Choose one living player to test for mysticism on N0. If Hidden Motives is attached, also tell the Wizard whether Inquisition is in play.', requires: ['private-information'], dependencyBarrier: 'information' },
    ]),
  role(ROLE.medium, 'Medium', FACTION.village, ['Information', 'Night'], [TRAIT.mystic],
    'Checks a dead player for corruption.', 'From N1, select one dead player and learn whether they were Corrupt.', [
      { id: `${ROLE.medium}.check`, name: 'Commune with the dead', kind: 'active', trigger: 'night.action', order: 30, activeFromNight: 1, target: { label: 'Dead player', min: 0, max: 1, selector: { kind: 'allPlayers', life: 'dead' }, allowNone: true }, effects: [{ type: 'inspectTrait', targets: { kind: 'chosen' }, trait: TRAIT.corrupt, positive: 'CORRUPT', negative: 'NOT CORRUPT' }], callout: 'check a dead player for corruption', instructions: 'Choose a dead player to test for corruption, or continue if no dead player can be checked.', requires: ['private-information'], dependencyBarrier: 'information' },
      { id: `${ROLE.medium}.spirit-check`, name: 'Check for a Spirit', kind: 'active', trigger: 'night.action', order: 31, activeFromNight: 1, publicRequirements: { match: 'all', requirements: [{ kind: 'traitPossible', trait: TRAIT.spirit }] }, target: { label: 'Dead player', min: 0, max: 1, selector: { kind: 'allPlayers', life: 'dead' }, allowNone: true }, effects: [{ type: 'conditional', condition: { op: 'hasStatus', subject: 'target', status: 'wherewolf.hidden-motives.status.known-spirit' }, effects: [{ type: 'removeStatus', targets: { kind: 'chosen' }, status: 'wherewolf.hidden-motives.status.spirit' }, { type: 'removeStatus', targets: { kind: 'chosen' }, status: 'wherewolf.hidden-motives.status.known-spirit' }, { type: 'announce', message: 'The known Spirit is removed from play.', visibility: 'moderator', category: 'Private result' }], otherwise: [{ type: 'inspectStatus', targets: { kind: 'chosen' }, status: 'wherewolf.hidden-motives.status.spirit', negative: 'NOT A SPIRIT' }, { type: 'conditional', condition: { op: 'hasStatus', subject: 'target', status: 'wherewolf.hidden-motives.status.spirit' }, effects: [{ type: 'addStatus', targets: { kind: 'chosen' }, status: { id: 'wherewolf.hidden-motives.status.known-spirit', name: 'Known Spirit' }, duration: 'permanent' }] }] }], callout: 'check a dead player for a Spirit', instructions: 'Choose a dead player. Reveal whether they are a Spirit and which Spirit role they have. Choosing a Spirit already identified by the Medium removes that Spirit from play.', requires: ['private-information'], dependencyBarrier: 'information' },
    ]),
  role(ROLE.witch, 'Witch', FACTION.village, ['Protection', 'Night'], [TRAIT.mystic],
    'Protects another player from shadow attacks.', 'Each repeating night, choose one other living player. Shadow attacks attempted against them that night are prevented before redirection can trigger. If Necromancer was announced as possible, also learn whether that player is Cursed.', [
      { id: `${ROLE.witch}.protect`, name: 'Protect from shadow attack', kind: 'active', trigger: 'night.action', order: 40, activeFromNight: 1, target: { label: 'Player to protect', min: 1, max: 1, selector: { kind: 'allPlayers', life: 'alive' }, excludeSelf: true }, effects: [
        { type: 'addStatus', targets: { kind: 'chosen' }, status: { id: 'wherewolf.base.status.shadow-protection', name: 'Protected from shadow attacks', traits: ['wherewolf.core.status.attack-protection'], data: { attackType: 'shadow' } }, duration: 'night' },
        { type: 'conditional', condition: { op: 'publicRolePossible', roleId: DARKEST_ROLE.necromancer }, effects: [{ type: 'inspectStatus', targets: { kind: 'chosen' }, status: 'wherewolf.darkest-night.status.curse', positive: 'CURSED', negative: 'NOT CURSED' }] },
      ], callout: 'protect a player from shadow attack', instructions: 'Choose one other living player to protect tonight. If Necromancer was announced as possible, also report whether that player is Cursed.', requires: ['shadow-attacks'], dependencyBarrier: 'protection' },
    ]),
  ...
  role(ROLE.priest, 'Priest', FACTION.village, ['Information', 'Setup'], [],
    'Locates the Sinner and learns whether the Seducer is present.', 'On setup, privately learn the Sinner’s identity and whether a Seducer is in the game.', [
      { id: `${ROLE.priest}.church`, name: 'Church knowledge', kind: 'active', trigger: 'setup.action', order: 40, effects: [{ type: 'learnRoleIdentity', roleId: ROLE.sinner }, { type: 'learnRolePresence', roleId: ROLE.seducer }], callout: 'learn the Sinner and whether the Seducer is present', instructions: 'Reveal the Sinner identity and Seducer presence privately.', requires: ['private-information'], dependencyBarrier: 'setup-information' },
    ]),
  ...
  role(ROLE.juliet, 'Juliet', FACTION.neutral, ['Relationship', 'Alternate Victory'], [],
    'Chooses Romeo and shares a Lovers victory.', 'On setup, choose Romeo. Romeo keeps their original role and ability, becomes a Lover, and is protected from shadow attacks while Juliet lives.', [
      { id: `${ROLE.juliet}.choose`, name: 'Choose Romeo', kind: 'active', trigger: 'setup.action', order: 60, target: { label: 'Romeo', min: 1, max: 1, selector: { kind: 'allPlayers', life: 'alive' }, excludeSelf: true }, effects: [{ type: 'linkRelationship', targets: { kind: 'chosen' }, relationship: 'wherewolf.base.relationship.romeo', reciprocal: 'wherewolf.base.relationship.juliet' }, { type: 'addStatus', targets: { kind: 'chosen' }, status: { id: ROLE.romeo, name: 'Romeo', traits: ['wherewolf.base.trait.lover'] }, duration: 'permanent' }, { type: 'changeFaction', targets: { kind: 'chosen' }, faction: FACTION.lovers }], callout: 'choose Romeo', instructions: 'Choose one other player as Romeo.', requires: ['relationships'], dependencyBarrier: 'relationships' },
  ...
  role(ROLE.guardian, 'Guardian Angel', FACTION.neutral, ['Protection', 'Relationship', 'Alternate Victory'], [TRAIT.guardian],
    'Guards one player, substitutes onto their Ballot, and retargets guardable attacks.', 'On setup, choose a guarded player. After that player qualifies for the Ballot, replace them as candidate. An otherwise successful guardable attack or kill is retargeted onto the Guardian, who is then an ordinary protectable target.', [
      { id: `${ROLE.guardian}.choose`, name: 'Choose guarded player', kind: 'active', trigger: 'setup.action', order: 50, target: { label: 'Player to guard', min: 1, max: 1, selector: { kind: 'allPlayers', life: 'alive' }, excludeSelf: true }, effects: [{ type: 'linkRelationship', targets: { kind: 'chosen' }, relationship: 'wherewolf.base.relationship.guarded', reciprocal: 'wherewolf.base.relationship.guardian' }], callout: 'choose a player to guard', instructions: 'Choose one other player to guard.', requires: ['relationships'], dependencyBarrier: 'relationships' },
      ...
      { id: `${ROLE.guardian}.redirect`, name: 'Guardable attack retarget', kind: 'passive', trigger: 'attack.successful', condition: { op: 'all', conditions: [{ op: 'targetIsRelationship', relationship: 'wherewolf.base.relationship.guarded' }, { op: 'event', field: 'guardable', compare: 'eq', value: true }] }, effects: [{ type: 'redirectEvent', targets: { kind: 'self' }, reason: 'Guardian Angel retarget', preventable: true }] },
      { id: `${ROLE.guardian}.redirect-kill`, name: 'Guardable kill retarget', kind: 'passive', trigger: 'kill.attempted', condition: { op: 'all', conditions: [{ op: 'targetIsRelationship', relationship: 'wherewolf.base.relationship.guarded' }, { op: 'event', field: 'guardable', compare: 'eq', value: true }] }, effects: [{ type: 'redirectEvent', targets: { kind: 'self' }, reason: 'Guardian Angel retarget', preventable: true }] },
...
export const BASE_SCENARIO: ScenarioDefinition = Object.freeze(withChecksum<ScenarioDefinition>({
  ...
  setupPipeline: [
    { id: 'base.setup.actions', type: 'role-actions', label: 'N0', trigger: 'setup.action' },
    { id: 'base.setup.news', type: 'announcements', label: 'First morning announcement', categories: ['Newsgiver'] },
