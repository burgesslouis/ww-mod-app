...
export const HIDDEN_MOTIVES_ROLES: RoleDefinition[] = [
  ...
  role(H.assassin, 'Assassin', FACTION.criminals, ['Criminal', 'Night'], [TRAIT.corrupt], 'Once per game, attacks during a possible Mystic turn.', 'From N1, name a publicly possible Mystic. If present, kill that role; if absent, choose another player to kill.', [
    ...
    night(`${H.assassin}.kill`, 'Assassination', 18, { label: 'Player killed by the Assassin', min: 0, max: 1, selector: { kind: 'allPlayers', life: 'alive' }, excludeSelf: true, allowNone: true }, [{ type: 'kill', targets: { kind: 'chosen' }, cause: 'Assassin' }], 'If the Assassin uses the once-per-game power, announce a publicly possible Mystic call and select the resulting victim.', { activeFromNight: 1, once: 'game' }),
  ]),
  role(H.guildMaster, 'Guild Master', FACTION.criminals, ['Criminal', 'Night'], [], 'Once per game tries to recruit through a possible Mystic turn.', 'From N1, names a publicly possible Mystic. If absent, it may approach another player; Guards and Werewolves kill the Guild Master, while Human City or Village targets become Criminal.', [
    ...
    night(`${H.guildMaster}.recruit`, 'Recruit', 19, { label: 'Player approached', min: 0, max: 1, selector: { kind: 'allPlayers', life: 'alive' }, excludeSelf: true, allowNone: true }, [{ type: 'conditional', condition: { op: 'any', conditions: [{ op: 'targetRoleHasTrait', trait: TRAIT.guard }, { op: 'targetRoleHasTrait', trait: TRAIT.werewolf }] }, effects: [{ type: 'kill', targets: { kind: 'self' }, cause: 'Failed Guild recruitment' }], otherwise: [{ type: 'changeFaction', targets: { kind: 'chosen' }, faction: FACTION.criminals }] }], 'If the Guild Master uses the power, announce the relevant possible Mystic call and select the player approached.', { activeFromNight: 1, once: 'game' }),
  ]),
  ...
  role(H.corruptGuard, 'Corrupt Guard', FACTION.anyShadow, ['Guard', 'Any Shadow'], [TRAIT.corrupt, TRAIT.guard, TRAIT.anyShadowWinner], 'Knows the Guards and Criminals and wins with any Shadow faction.', 'On N0, recognises all Guards and Criminals.', [setup(`${H.corruptGuard}.intro`, 'Meet Guards and Criminals', 120, [{ type: 'learnPlayers', targets: { kind: 'trait', trait: TRAIT.guard, life: 'alive' }, label: 'Guards' }, { type: 'learnPlayers', targets: { kind: 'faction', faction: FACTION.criminals, life: 'alive' }, label: 'Criminals' }], 'Show the Corrupt Guard all Guards and Criminals.')]),
