...
function role(id: string, name: string, faction: string, categories: string[], traits: string[], summary: string, description: string, abilities: AbilityDefinition[] = [], state: StateVariable[] = [], maximumCopies = 1, statuses?: RoleDefinition['statuses'], displayTeam?: string): RoleDefinition {
  ...
  return withChecksum({
    ...
    faction, factionWinScope: faction === FACTION.anyShadow || faction === FACTION.anyHuman || new Set<string>([FACTION.inquisition, FACTION.criminals, FACTION.city]).has(faction) ? 'alignment' : 'exact', ...(displayTeam ? { displayTeam } : {}), categories, traits,
...
function withFactionRecommendations(roleDefinition: RoleDefinition, factionRecommendations: NonNullable<RoleDefinition['factionRecommendations']>): RoleDefinition {
  return withChecksum({ ...roleDefinition, factionRecommendations })
}
...
], 'Wake the role with the Wolf Pack. Wolves recognize one another, then identify the Defector and the Hag. The Hag does not wake for this call.', { id: 'wherewolf.base.group.wolf-introduction', label: 'Wolf Pack' })
...
const outcastBite = night('wherewolf.base.ability.wolf-bite', 'Join the Pack bite', 51,
  ...
  [{ type: 'queueAttack', targets: { kind: 'chosen' }, attackType: 'shadow', modifiers: { guardable: true, healable: true, protectable: true, notifyTargetOnHit: true } }], 'Wake with the strongest biting Werewolf and agree the Pack target.',
  { activeFromNight: 1, kind: 'shared-faction', simultaneous: { id: 'wherewolf.darkest-night.group.pack-bite', label: 'Biting Werewolves' } })
...
export const DARKEST_NIGHT_ROLES: RoleDefinition[] = [
  ...
  role(D.loneWolf, 'Lone Wolf', FACTION.loneWolf, ['Shadow', 'Night', 'Alternate Victory'], [TRAIT.werewolf, TRAIT.corrupt, TRAIT.shadow],
    'Takes the bite only after the Wolf Pack is gone.', 'Meets the wolves, Defector, and Hag. Other Werewolves cannot kill it. It follows its own Shadow victory.', [
      wolfIntro,
      night(`${D.loneWolf}.bite`, 'Lone bite', 54, { label: 'Bite target', min: 0, max: 1, selector: { kind: 'allPlayers', life: 'alive' }, excludeSelf: true, allowNone: true }, [{ type: 'queueAttack', targets: { kind: 'chosen' }, attackType: 'shadow', modifiers: { guardable: true, healable: true, protectable: true, notifyTargetOnHit: true } }], 'If no Wolf Pack member lives, choose one player to bite.', { condition: { op: 'count', selector: { kind: 'faction', faction: FACTION.wolves, life: 'alive' }, compare: 'eq', value: 0 } }),
  ...
  role(D.hag, 'Hag', FACTION.anyShadow, ['Any Shadow', 'Mystic'], [TRAIT.corrupt, TRAIT.mystic, TRAIT.anyShadowWinner], 'Hexes Mystics who choose it.', 'During the initial role-introduction night, every Shadow role identifies the Hag. The Hag does not identify the Shadow roles. A Mystic who targets the Hag receives negative information and cannot protect while the Hag lives.', [
  ...
  role(D.vampire, 'Vampire', FACTION.vampire, ['Undead', 'Transformation'], [TRAIT.shadow, TRAIT.corrupt, TRAIT.undead], 'Turns a non-Mystic player into a Minion.', 'From N1, bites one living player. A Werewolf or Vampire Hunter target causes undead backlash instead.', [
    ...
    ], 'Choose a living player. Werewolves and the Vampire Hunter cause backlash; Mystics resist; another target becomes a Minion.', { activeFromNight: 1 }),
  ]),
  ...
  role(D.nosferatu, 'Nosferatu', FACTION.nosferatu, ['Undead', 'Revival'], [TRAIT.shadow, TRAIT.corrupt, TRAIT.undead], 'Raises one eligible night victim as a Thrall.', 'After attacks from N1 onward, chooses someone killed that night. Werewolves and Vampire Hunters cause backlash.', [
    ...
    ], 'Choose an eligible player killed tonight, or nobody. Resolve backlash before raising a Thrall.', { activeFromNight: 1, dependencyBarrier: 'after-attack-resolution' }),
  ]),
  ...
  role(D.igor, 'Igor', FACTION.undeadSupport, ['Undead support'], [TRAIT.corrupt, TRAIT.undeadSupport], 'Supports both Vampire and Nosferatu.', 'Igor meets both leaders and wins with either faction. If a leader suffers backlash after targeting a Wolf or Hunter, Igor dies instead. If neither supported undead faction can still win, Igor leaves the game as a loser the following morning.', [
    setup(`${D.igor}.vampire`, 'Protect the Vampire', 97, [{ type: 'learnRoleIdentity', roleId: D.vampire }, { type: 'learnRoleIdentity', roleId: D.hag }, { type: 'linkRelationship', targets: { kind: 'role', roleId: D.vampire, life: 'any' }, relationship: 'wherewolf.darkest-night.relationship.protect-vampire', reciprocal: 'wherewolf.darkest-night.relationship.igor-vampire' }], 'Identify the Vampire and the Hag, if present.'),
    ...
    { id: `${D.igor}.unsupported`, name: 'No surviving patron', kind: 'passive', trigger: 'morning.beforeVictory', condition: { op: 'all', conditions: [
      { op: 'isAlive', subject: 'self' },
      { op: 'any', conditions: [{ op: 'count', selector: { kind: 'role', roleId: D.vampire, life: 'any' }, compare: 'gt', value: 0 }, { op: 'count', selector: { kind: 'role', roleId: D.nosferatu, life: 'any' }, compare: 'gt', value: 0 }] },
      { op: 'count', selector: { kind: 'faction', faction: FACTION.vampire, life: 'alive' }, compare: 'eq', value: 0 },
      { op: 'count', selector: { kind: 'faction', faction: FACTION.nosferatu, life: 'alive' }, compare: 'eq', value: 0 },
    ] }, effects: [{ type: 'personalLose', targets: { kind: 'self' }, reason: 'neither supported undead faction can still win' }, { type: 'kill', targets: { kind: 'self' }, cause: 'neither supported undead faction can still win', reportAsNightDeath: true, spiritEligible: false }] },
  ]),
  role(D.vampireHunter, 'Vampire Hunter', FACTION.village, ['Information', 'Protection'], [TRAIT.undeadHunter], 'Detects Vampire and Nosferatu and kills an undead attacker.', 'On N0, learns whether each undead leader is present. A leader targeting it suffers backlash.', [
  ...
  role(D.necromancer, 'Necromancer', FACTION.necromancer, ['Shadow', 'Curse', 'Alternate Victory'], [TRAIT.shadow, TRAIT.corrupt, TRAIT.mystic], 'Curses two players and threatens a one-morning ritual.', 'Cursed players appear Corrupt and receive one extra vote in both tallies. When the last Curse is gone, announce the ritual; if it is not stopped, Necromancer wins.', [
    { ...setup(`${D.necromancer}.curse`, 'Place two Curses', 70, [{ type: 'learnRoleIdentity', roleId: D.hag }, { type: 'addStatus', targets: { kind: 'chosen' }, status: { id: 'wherewolf.darkest-night.status.curse', name: 'Curse', traits: [TRAIT.corrupt], data: { sourceMustLive: true }, abilities: [{ id: 'wherewolf.darkest-night.status.curse.vote', name: 'Curse vote', kind: 'status', trigger: 'vote.beforeTally', condition: { op: 'targetIsSelf' }, effects: [{ type: 'modifyVotesReceived', targets: { kind: 'self' }, operation: 'add', value: 1 }] }] }, duration: 'permanent' }], 'Identify the Hag, then choose two different players to Curse.'), target: { label: 'Players to Curse', min: 2, max: 2, selector: { kind: 'allPlayers', life: 'alive' }, excludeSelf: true, distinct: true } },
  ...
  role(D.undertaker, 'Undertaker', FACTION.necromancer, ['Information'], [], 'Supports the Necromancer and knows whether it is present.', 'The Undertaker wins with the Necromancer and learns whether it is present on Night 0. If the Necromancer dies, the Undertaker leaves the game as a loser the following morning.', [
    setup(`${D.undertaker}.check`, 'Check for Necromancer', 37, [{ type: 'learnRolePresence', roleId: D.necromancer }, { type: 'learnRoleIdentity', roleId: D.hag }], 'Identify the Hag and tell the Undertaker whether Necromancer is present.'),
    { id: `${D.undertaker}.patron-dead`, name: 'Follow the Necromancer', kind: 'passive', trigger: 'morning.beforeVictory', condition: { op: 'all', conditions: [
      { op: 'isAlive', subject: 'self' },
      { op: 'count', selector: { kind: 'role', roleId: D.necromancer, life: 'any' }, compare: 'gt', value: 0 },
      { op: 'count', selector: { kind: 'role', roleId: D.necromancer, life: 'alive' }, compare: 'eq', value: 0 },
    ] }, effects: [{ type: 'personalLose', targets: { kind: 'self' }, reason: 'the Necromancer is dead' }, { type: 'kill', targets: { kind: 'self' }, cause: 'the Necromancer is dead', reportAsNightDeath: true, spiritEligible: false }] },
  ]),
  role(D.possessed, 'Possessed', FACTION.possessed, ['Shadow', 'Transformation'], [TRAIT.shadow, TRAIT.corrupt, TRAIT.possessed], 'Passes possession to a chosen successor when killed outside a burn.', 'Choose a possible successor on N0. If the Possessed later dies other than by burning, that player becomes Possessed.', [
    { ...setup(`${D.possessed}.successor`, 'Choose a successor', 75, [{ type: 'learnRoleIdentity', roleId: D.hag }, { type: 'linkRelationship', targets: { kind: 'chosen' }, relationship: 'wherewolf.darkest-night.relationship.possessed-successor' }], 'Identify the Hag, then choose the player who will inherit possession if this Possessed dies.'), target: { label: 'Possession successor', min: 1, max: 1, selector: { kind: 'allPlayers', life: 'alive' }, excludeSelf: true } },
  ...
  role(D.amnesiac, 'Amnesiac', FACTION.neutral, ['Transformation'], [TRAIT.amnesiac], 'Receives a publicly possible role on N1.', 'On N1, the moderator chooses its actual role from the roles announced as possible at setup. It loses if burned first.', [
    night(`${D.amnesiac}.remember`, 'Choose the remembered role', 8, { label: 'Publicly possible role', min: 1, max: 1, selector: { kind: 'publicPossibleRoles' } }, [{ type: 'transformRole', targets: { kind: 'self' }, roleId: { chosenRole: true } }], 'Choose the Amnesiac’s real role from the roles announced as possible at the start.', { activeFromNight: 1, once: 'game' }),
...
const littleFolkIntro = setup('wherewolf.hidden-motives.ability.little-folk-intro', 'Meet the Little Folk', 105, [{ type: 'learnPlayers', targets: { kind: 'trait', trait: TRAIT.littleFolk, life: 'alive' }, label: 'Little Folk' }, { type: 'conditional', condition: { op: 'hasTrait', subject: 'actor', trait: TRAIT.anyShadowWinner }, effects: [{ type: 'learnRoleIdentity', roleId: D.hag }] }], 'Wake all Little Folk together and recognize one another. The Goblin also identifies the Hag; the Hag does not wake for this call.', { id: 'wherewolf.hidden-motives.group.little-folk', label: 'Little Folk' })
...
export const HIDDEN_MOTIVES_ROLES: RoleDefinition[] = [
  role(H.inquisitor, 'Inquisitor', FACTION.inquisition, ['Inquisition', 'Information'], [], 'Finds the Inquisition and counts living Mystics.', 'On N0, learns the Templar, Executioner, and Mystic count. It is alerted whenever a Mystic targets it.', [
  ...
  role(H.assassin, 'Assassin', FACTION.criminals, ['Criminal', 'Night'], [TRAIT.corrupt], 'Once per game, attacks during a possible Mystic turn.', 'From N1, name a publicly possible Mystic. If present, kill that role; if absent, choose another player to kill.', [
    ...
    night(`${H.assassin}.kill`, 'Assassination', 18, { label: 'Player killed by the Assassin', min: 0, max: 1, selector: { kind: 'allPlayers', life: 'alive' }, excludeSelf: true, allowNone: true }, [{ type: 'kill', targets: { kind: 'chosen' }, cause: 'Assassin', modifiers: { guardable: true } }], 'If the Assassin uses the once-per-game power, announce a publicly possible Mystic call and select the resulting victim.', { activeFromNight: 1, once: 'game', turnDependency: { anchors: { kind: 'roleTrait', trait: TRAIT.mystic }, placement: 'before', requiresSpokenCall: true } }),
  ]),
  role(H.guildMaster, 'Guild Master', FACTION.criminals, ['Criminal', 'Night'], [], 'Once per game tries to recruit through a possible Mystic turn.', 'From N1, names a publicly possible Mystic. If absent, it may approach another player; Guards and Werewolves kill the Guild Master, while Human City or Village targets become Criminal.', [
    ...
    night(`${H.guildMaster}.recruit`, 'Recruit', 19, { label: 'Player approached', min: 0, max: 1, selector: { kind: 'allPlayers', life: 'alive' }, excludeSelf: true, allowNone: true }, [{ type: 'conditional', condition: { op: 'any', conditions: [{ op: 'targetRoleHasTrait', trait: TRAIT.guard }, { op: 'targetRoleHasTrait', trait: TRAIT.werewolf }] }, effects: [{ type: 'kill', targets: { kind: 'self' }, cause: 'Failed Guild recruitment' }], otherwise: [{ type: 'changeFaction', targets: { kind: 'chosen' }, faction: FACTION.criminals }] }], 'If the Guild Master uses the power, announce the relevant possible Mystic call and select the player approached.', { activeFromNight: 1, once: 'game', turnDependency: { anchors: { kind: 'roleTrait', trait: TRAIT.mystic }, placement: 'before', requiresSpokenCall: true } }),
  ]),
  role(H.spy, 'Spy', FACTION.criminals, ['Criminal', 'Voting'], [], 'Knows the Criminals but must cast its own vote.', 'The Spy recognises the Criminal faction on N0. Record the aggregate vote normally; the moderator enforces its required self-vote.', [setup(`${H.spy}.intro`, 'Meet the Criminals', 117, [{ type: 'learnFactionMembers', faction: FACTION.criminals }], 'Show the Spy the other Criminals.')]),
  role(H.thief, 'Thief', FACTION.criminals, ['Criminal', 'Protection'], [], 'Can interrupt one possible Mystic action and has one Shadow protection.', 'The Thief recognises Criminals, can suppress one player’s night action once, and survives the first Shadow attack.', [
    ...
    night(`${H.thief}.interrupt`, 'Interrupt a role', 9, { label: 'Player whose night action is interrupted', min: 0, max: 1, selector: { kind: 'allPlayers', life: 'alive' }, allowNone: true }, [{ type: 'suppressAction', targets: { kind: 'chosen' }, trigger: 'night.action', duration: 'night' }], 'If the Thief uses the once-per-game interruption, name the public role call and select the affected player.', { once: 'game', turnDependency: { anchors: { kind: 'roleTrait', trait: TRAIT.mystic }, placement: 'before', requiresSpokenCall: true } }),
  ]),
  withFactionRecommendations(role(H.corruptGuard, 'Corrupt Guard', FACTION.anyShadow, ['Guard', 'Any Shadow'], [TRAIT.corrupt, TRAIT.guard, TRAIT.anyShadowWinner], 'Knows the Guards and Criminals and wins with any Shadow faction.', 'On N0, recognizes all Guards and Criminals and identifies the Hag.', [setup(`${H.corruptGuard}.intro`, 'Meet Guards and Criminals', 120, [{ type: 'learnPlayers', targets: { kind: 'trait', trait: TRAIT.guard, life: 'alive' }, label: 'Guards' }, { type: 'learnPlayers', targets: { kind: 'faction', faction: FACTION.criminals, life: 'alive' }, label: 'Criminals' }, { type: 'learnRoleIdentity', roleId: D.hag }], 'Recognize the Guards and Criminals and identify the Hag.')]), [
    { faction: FACTION.criminals, when: { match: 'all', requirements: [{ kind: 'factionPossible', faction: FACTION.criminals }] }, priority: 1, autoDefault: true, reason: 'Criminals are publicly possible in this setup.' },
  ]),
  role(H.guard, 'Guard', FACTION.village, ['Guard', 'Information'], [TRAIT.guard], 'Knows other Guards and the Criminal count.', 'On N0, recognises other Guards and learns the number of Criminals.', [setup(`${H.guard}.intro`, 'Meet the Guards', 121, [{ type: 'learnPlayers', targets: { kind: 'trait', trait: TRAIT.guard, life: 'alive' }, label: 'Guards' }, { type: 'learnCount', targets: { kind: 'faction', faction: FACTION.criminals, life: 'alive' }, label: 'Criminals' }], 'Wake all Guards together and tell them the number of Criminals.')], [], 2),
  ...
  withFactionRecommendations(role(H.goblin, 'Goblin', FACTION.anyShadow, ['Little Folk', 'Any Shadow'], [TRAIT.littleFolk, TRAIT.corrupt, TRAIT.anyShadowWinner], 'Wins with any Shadow faction while another Little Folk survives.', 'Mystic powers have no effect on living Little Folk. The last living Little Folk dies the following morning.', [littleFolkIntro, littleFolkDeath]), [
    { faction: FACTION.wolves, when: { match: 'all', requirements: [{ kind: 'factionPossible', faction: FACTION.wolves }] }, priority: 1, reason: 'The Wolf Pack is publicly possible in this setup.' },
  ]),
...
export const OFFICIAL_SCENARIO: ScenarioDefinition = withChecksum({
  ...
  setupPipeline: [
    { id: 'official.setup.actions', type: 'role-actions', label: 'N0', trigger: 'setup.action', abilityIds: setupIds },
    { id: 'official.setup.news', type: 'announcements', label: 'First morning announcement', categories: ['Newsgiver'] },
    { id: 'official.setup.complete', type: 'cycle-end', label: 'Finish setup' },
  ],
  cyclePipeline: [
