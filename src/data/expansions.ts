...
export const DARKEST_NIGHT_ROLES: RoleDefinition[] = [
  ...
  role(D.hag, 'Hag', FACTION.anyShadow, ['Any Shadow', 'Mystic'], [TRAIT.corrupt, TRAIT.mystic, TRAIT.anyShadowWinner], 'Hexes Mystics who choose it.', 'During the initial role-introduction night, every role carrying the Shadow trait identifies the Hag. The Hag does not identify the Shadow roles. A Mystic who targets the Hag receives negative information and cannot protect while the Hag lives.', [
  ...
  role(D.undertaker, 'Undertaker', FACTION.necromancer, ['Information'], [], 'Supports the Necromancer and knows whether it is present.', 'The Undertaker wins with the Necromancer and learns whether it is present on Night 0. If the Necromancer dies, the Undertaker leaves the game as a loser the following morning.', [
    setup(`${D.undertaker}.check`, 'Check for Necromancer', 37, [{ type: 'learnRolePresence', roleId: D.necromancer }], 'Tell the Undertaker whether Necromancer is present.'),
...
export const HIDDEN_MOTIVES_ROLES: RoleDefinition[] = [
  ...
  withFactionRecommendations(role(H.corruptGuard, 'Corrupt Guard', FACTION.anyShadow, ['Guard', 'Any Shadow'], [TRAIT.corrupt, TRAIT.guard, TRAIT.anyShadowWinner], 'Knows the Guards and Criminals and wins with any Shadow faction.', 'On N0, recognises all Guards and Criminals.', [setup(`${H.corruptGuard}.intro`, 'Meet Guards and Criminals', 120, [{ type: 'learnPlayers', targets: { kind: 'trait', trait: TRAIT.guard, life: 'alive' }, label: 'Guards' }, { type: 'learnPlayers', targets: { kind: 'faction', faction: FACTION.criminals, life: 'alive' }, label: 'Criminals' }], 'Show the Corrupt Guard all Guards and Criminals.')]), [
